...
describe('ProductService - Unit Tests', () => {
    ...
    let mockManufacturerRepository;
    let mockCategoryRepository;
    const mockManufacturer = new Manufacturer('manufacturer-id-1', 'Test Manufacturer', 'Test Description');
    const mockCategory = new Category('category-id-1', 'Test Category');
    const mockProduct = new Product('product-id-1', 'ART-123456', 'Test Product', mockManufacturer, mockCategory, 'Test Description', 100.5, true, 4.5);
    beforeEach(() => {
        ...
        productService = new ProductService(mockProductRepository, mockManufacturerRepository, mockCategoryRepository);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('addProduct', () => {
        ...
        it('должен выбросить ошибку если производитель не найден', async () => {
            ...
            await expect(productService.addProduct(dto)).rejects.toThrow('Manufacturer with id non-existent-manufacturer not found');
            expect(mockManufacturerRepository.getManufById).toHaveBeenCalledWith('non-existent-manufacturer');
        ...
        it('должен выбросить ошибку если категория не найдена', async () => {
            ...
            await expect(productService.addProduct(dto)).rejects.toThrow('Category with id non-existent-category not found');
        ...
        it('должен использовать рейтинг по умолчанию 0 если не указан', async () => {
            ...
            const productWithoutRating = new Product('product-id-1', 'ART-123456', 'Test Product', mockManufacturer, mockCategory, 'Test Description', 100.5, true, 0);
            mockManufacturerRepository.getManufById.mockResolvedValue(mockManufacturer);
            mockCategoryRepository.getCategoryById.mockResolvedValue(mockCategory);
    ...
    describe('updateProduct', () => {
        it('должен успешно обновить продукт', async () => {
            ...
            const updatedProduct = new Product('product-id-1', 'ART-123456', 'Updated Product Name', mockManufacturer, mockCategory, 'Test Description', 200.75, true, 4.5);
            mockProductRepository.getProductById.mockResolvedValue(mockProduct);
        ...
        it('должен выбросить ошибку если продукт не найден', async () => {
            ...
            mockProductRepository.getProductById.mockResolvedValue(null);
            await expect(productService.updateProduct('non-existent-id', updateDto)).rejects.toThrow('Product with id non-existent-id not found');
        ...
        it('должен обновить производителя если указан новый manufacturerId', async () => {
            const newManufacturer = new Manufacturer('manufacturer-id-2', 'New Manufacturer', 'New Description');
            const updateDto = {
                manufacturerId: 'manufacturer-id-2',
            };
            mockProductRepository.getProductById.mockResolvedValue(mockProduct);
            mockManufacturerRepository.getManufById.mockResolvedValue(newManufacturer);
            const updatedProduct = new Product('product-id-1', 'ART-123456', 'Test Product', newManufacturer, mockCategory, 'Test Description', 100.5, true, 4.5);
            mockProductRepository.updateProduct.mockResolvedValue(updatedProduct);
            const result = await productService.updateProduct('product-id-1', updateDto);
        ...
        it('должен обновить категорию если указан новый categoryId', async () => {
            ...
            const updatedProduct = new Product('product-id-1', 'ART-123456', 'Test Product', mockManufacturer, newCategory, 'Test Description', 100.5, true, 4.5);
            mockProductRepository.updateProduct.mockResolvedValue(updatedProduct);
            const result = await productService.updateProduct('product-id-1', updateDto);
        ...
        it('должен выбросить ошибку если новый производитель не найден', async () => {
            ...
            mockProductRepository.getProductById.mockResolvedValue(mockProduct);
            mockManufacturerRepository.getManufById.mockResolvedValue(null);
            await expect(productService.updateProduct('product-id-1', updateDto)).rejects.toThrow('Manufacturer with id non-existent-manufacturer not found');
        });
    });
    describe('updateAvailability', () => {
        it('должен успешно обновить доступность продукта', async () => {
            const updatedProduct = new Product('product-id-1', 'ART-123456', 'Test Product', mockManufacturer, mockCategory, 'Test Description', 100.5, false, 4.5);
            mockProductRepository.getProductById.mockResolvedValue(mockProduct);
            mockProductRepository.updateProduct.mockResolvedValue(updatedProduct);
        ...
        it('должен выбросить ошибку если продукт не найден', async () => {
            mockProductRepository.getProductById.mockResolvedValue(null);
            await expect(productService.updateAvailability('non-existent-id', true)).rejects.toThrow('Product with id non-existent-id not found');
        });
    });
...
//# sourceMappingURL=ProductService.unit.test.js.map