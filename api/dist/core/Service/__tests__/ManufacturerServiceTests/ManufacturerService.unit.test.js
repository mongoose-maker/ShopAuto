...
describe('ManufacturerService - Unit Tests', () => {
    ...
    describe('addManufacturer', () => {
        ...
        it('должен создать производителя с правильным именем', async () => {
            ...
            expect(mockManufacturerRepository.addManuf).toHaveBeenCalledWith(expect.objectContaining({
                name: 'Toyota',
            }));
        });
    });
    ...
    describe('updateManufacturerInfo', () => {
        it('должен успешно обновить производителя', async () => {
            ...
            const updatedManufacturer = new Manufacturer(manufacturerId, 'updated-manufacturer-name', 'updated-manufacturer-description');
            ...
            expect(mockManufacturerRepository.updateManufInfo).toHaveBeenCalledWith(manufacturerId, expect.objectContaining({
                name: 'updated-manufacturer-name',
                descriptionManufacturer: 'updated-manufacturer-description',
            }));
        ...
        it('должен выбросить ошибку если id не указан', async () => {
            ...
            await expect(manufacturerService.updateManufacturerInfo('', updateDto)).rejects.toThrow('Manufacturer ID is required');
        });
    });
    describe('updateManufacturerProducts', () => {
        it('должен успешно обновить список продуктов производителя', async () => {
            ...
            const updatedManufacturer = new Manufacturer(manufacturerId, 'Test Manufacturer', 'Test Description');
            mockManufacturerRepository.updateListProductByManuf.mockResolvedValue(updatedManufacturer);
            const result = await manufacturerService.updateManufacturerProducts(manufacturerId, updateDto);
            expect(mockManufacturerRepository.updateListProductByManuf).toHaveBeenCalledWith(manufacturerId, ['product-1', 'product-2', 'product-3']);
            expect(result).toEqual(updatedManufacturer);
        });
        it('должен вернуть null если производитель не найден', async () => {
        ...
        it('должен выбросить ошибку если id не указан', async () => {
            ...
            await expect(manufacturerService.updateManufacturerProducts('', updateDto)).rejects.toThrow('Manufacturer ID is required');
        });
        it('должен выбросить ошибку если продуктов больше 100', async () => {
            ...
            await expect(manufacturerService.updateManufacturerProducts(manufacturerId, updateDto)).rejects.toThrow('Manufacturer cannot have more than 100 products');
        });
    });
...
//# sourceMappingURL=ManufacturerService.unit.test.js.map