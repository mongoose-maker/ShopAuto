...
export class OrderService {
  ...
  async createOrder(dto: AddOrderDto): Promise<Order> {
    ...
    for (const itemDto of dto.items) {
      ...
      const orderItem = new OrderItem(
        undefined,
        undefined, // orderId будет установлен после создания заказа
        itemDto.productId,
        itemDto.quantity,
        unitPrice,
        totalPrice
    ...
    if (Math.abs(calculatedTotal - dto.totalAmount) > 0.01) {
      throw new Error(
        `Total amount mismatch: calculated ${calculatedTotal}, provided ${dto.totalAmount}`
      );
    }

    // Создать заказ (id будет создан в репозитории)
    const order = new Order(
      undefined, // id будет создан в репозитории
      dto.userId,
      orderItems,
      "created",
      dto.totalAmount,
  ...
  async updateOrderStatus(
    ...
    if (!ORDER_STATUSES.includes(status)) {
      throw new Error(
        `Invalid status: ${status}. Valid statuses: ${ORDER_STATUSES.join(
          ", "
        )}`
      );
    }
